...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}[1]
    \Procedure{Plan}{$systemState$}
    	...
    	\If{$scalingUtility$ > 0~AND~$requestRate$ > 2} 
    		\State $scaleOut \gets true$
    		\If{$spike$}
    			\State $addVMs \gets$ \Call{Ceil}{$currentVMs * 3$}
    		\Else
    			\State $addVMs \gets$ \Call{Ceil}{$currentVMs * 0.5$}
    		\EndIf
    	\ElsIf{$scalingUtility < -0.5$}
