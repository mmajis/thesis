...
load balancer (nginx version x.x.x\todo{check nginx version}). The requests
...
\missingfigure{deployment diagram}

\todo{this stuff about prototype elasticity preferences may be better to put in the results chapter?}
...
1.0 down to zero as the response time grows from 0.8 to 1.5 seconds. \todo{math
notation for preference and utility functions in practice?} The response time is
...
The weights $w_{i}^{in}$ and $w_{i}^{out}$ are distributed among these two groups so that $\sum\limits_{i=1}^j w_{i}^{in} = 1$ and $\sum\limits_{i=1}^k w_{i}^{out} = 1$.



In addition to the scaling function, the elasticity controller detects trends in
response time by keeping track of the request response time in 5-minute moving
window. The slope of response time is is calculated based on the metric value 5
minutes ago and currently. If the slope is above a
...
to $U(X)$.\todo{What to do to get -1 value if the WEIGHT of cost is less than 
one? Normalize?} 
...
The controller application delegates \textit{execution} of the plan to a
...
\missingfigure{make a table of server apps / support apps and their versions.}
