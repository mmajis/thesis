...
are given in table \ref{table:componentVersions}.
...
in chapter~\ref{sec:elasticityController}.
...
\subsection{Elasticity controller}\label{sec:elasticityController}
...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}[1]
    \Repeat{ every $loopInterval$ seconds}
    	\State $monitorData \gets$ \Call{Monitor}{\null} 
    	\State $systemState \gets$ \Call{Analyze}{$monitorData$} 
    	\If{$systemState.utility < utilityThreshold$}
    		\State $scalingPlan \gets$ \Call{Plan}{$systemState$}
	    	\State \Call{Execute}{$scalingPlan$}
    	\EndIf
...
seconds (\textit{loopInterval} in Algorithm~\ref{alg:scalingDecision}) and
calculates the utility based on the preference functions defined for the
business application. The controller first analyzes the monitoring data
together with other \textit{knowledge} of the system to determine the current
system state. If utility (eq. \ref{eq:utilityfunction} on
page~\pageref{eq:utilityfunction}) is below a threshold of 0.7
(\textit{utilityThreshold} in Algorithm~\ref{alg:scalingDecision}), the
controller proceeds to the planning phase.
...
threshold parameters \textit{outThold} and \textit{inThold}. The amount of
server instances to add or remove depends on the system state. With gradual
growth, the amount of application server nodes is increased by half
(\textit{nonSpikeMultiplier}). If the response time slope indicates a spike is
underway, the amount of server instances to add is the current amount
multiplied by three (\textit{spikeMultiplier}). For scaling in, the amount is
decided with the use of the cost preference function $P_{c}$
(eq.~\ref{eq:costPreference} on page~\pageref{eq:costPreference}). The cost
preference is calculated with decreasing instance count until it reaches the
maximum preference value. Any instances above the count which yields maximum
cost preference are terminated.

Further thresholds apply to the planning phase. In order to scale out, the
request rate needs to be larger than 2 requests/s (\textit{rrThold} in
Algorithm~\ref{alg:scalingDecisionPlan}). To avoid repeatedly adjusting the
size of the deployment, scaling in requires the scaling utility function value
to be $S(X) < -0.5$, i.e. \textit{inThold} is set to $0.5$.

\begin{algorithm}[!h]
  \caption{The Planning phase of the MAPE-K loop in detail.}\label{alg:scalingDecisionPlan}
  \begin{algorithmic}[1]
    \Procedure{Plan}{$systemState$}
    	\State $scalingUtility\gets$ \Call{S}{$systemState$} 
    	\Comment{See eq.~\ref{eq:scalingfunction} on 
    	page~\pageref{eq:scalingfunction}}
    	\If{$scalingUtility > outThold$~AND~$requestRate$ > $rrThold$} 
    		\State $scaleOut \gets true$
    		\If{$spike$}
    			\State $addVMs \gets$ \Call{Ceil}{$currentVMs * spikeMultiplier$}
    		\Else
    			\State $addVMs \gets$ \Call{Ceil}{$currentVMs * nonSpikeMultiplier$}
    		\EndIf
    	\ElsIf{$scalingUtility < inThold$}
    		\State $scaleIn \gets true$
    		\State $removeVMs \gets currentVMs$
    		\While{\Call{$P_{c}$}{$removeVMs$} < 1} \Comment{See eq.~\ref{eq:costPreference} on page~\pageref{eq:costPreference}}
...
parameters used in algorithms~\ref{alg:scalingDecision}
and~\ref{alg:scalingDecisionPlan}. The tests described in
chapter~\ref{sec:results} were carried out with values chosen based on
empirical tuning over 30 preliminary test runs for the application under test.
Table~\ref{table:knobsToTurn} lists the parameters, the values used in the
reported tests and a summary of each parameter's effect on the elasticity
controller's behavior.

	\begin{table}[h]
		\resizebox{\textwidth}{!}{
	    \begin{tabular}{| m{3cm} | r | m{7cm} |}
	    \hline
	    \multicolumn{3}{| c |}{\textbf{Scaling decision parameters}} \\ \hline
		\textbf{Parameter} & \textbf{Value used} & \textbf{Purpose} \\ \hline
		\textit{loopInterval} & 20 s & The frequency at which the decision loop is 
		executed. 
		This needs to be larger than the monitoring system's update frequency.\\
		 \hline
		\textit{utilityThreshold} & 0.7 & The value of $U(X)$ below which planning 
		of a 
		scaling operation is started. \\ \hline
	    \textit{outThold} & 0.0 & Scaling out is planned if $S(X)$ is greater than 
	    this value.
	     \\ \hline
	    \textit{inThold} & -0.5 & Scaling in is planned if $S(X)$ is less than this 
	    value. \\ 
	    \hline
	    \textit{rrThold} & 2.0 & Scaling out is planned if $requestRate$ is greater 
	    than this 
	    value. \\ \hline
	    \textit{spike} & $200 \frac{ms}{min}$ & The controller considers the system 
	    to be 
	    under a spike load situation if the request rate slope $k_{r}$ is greater than this 
	    value. \\ \hline 
	    \textit{spikeMultiplier} & 3.0 & The spike multiplier determines how many 
	    new virtual 
	    machines are added when scaling out in a spike situation \\ \hline
	  	\textit{nonSpikeMultiplier} & 0.5 & The non spike multiplier determines how 
	  	many new virtual machines are added when scaling out under normal load. \\ 
	  	\hline
	    $P_{r}(X)$ & See eq.~\ref{eq:responseTimePreference} & The response time 
	    preference function determines the business stakeholder preference value for 
	    a given response time. \\ \hline
	    $P_{c}(X)$ & See eq.~\ref{eq:costPreference} & The cost preference function 
	    determines the business stakeholder preference value for a given cost per 
	    request value. \\ \hline
	    Other preference functions & none & Further preference functions can be 
	    included in the calculation of utility $U(X)$ and scaling utility $S(X)$ to refine the controller's behavior under varying business requirements. \\ 
	    \hline
	    \end{tabular}
		}
	    \caption{Parameters which influence the elasticity controller's scaling decisions.}
	    \label{table:knobsToTurn}
	\end{table}
